...
public class ModelsFolder {
    ...
    public static void initializeConfig() {
        ...
        if (!file.exists()) {
            MessageUtil.log("Failed to create models directory!");
            return;
        }

        if (!file.isDirectory()) {
            MessageUtil.log("Directory models was not a directory!");
            return;
        }
        ...
        try {
            FileUtils.writeStringToFile(
                    new File(MetadataHandler.PLUGIN.getDataFolder().getAbsolutePath() + File.separatorChar + "output"
                            + File.separatorChar + "Daedalus" + File.separatorChar + "assets" + File.separatorChar +
            ...
            MessageUtil.log("Failed to generate the iron horse armor file!");
        ...
        for (Map.Entry<String, List<FileModelConverter>> entry : mappedModels.entrySet()) {
            ...
            for (FileModelConverter fileModelConverter : models) {
                for (BoneBlueprint boneBlueprint : fileModelConverter.getSkeletonBlueprint().getBoneMap().values()) {
                    if (boneBlueprint.getBoneName().contains("daedalus_autogenerated_root")) continue;
                    ...
                    modelContentsJson.put("model", "erethon:" + boneBlueprint.getBoneName().split(":")[1]);
        ...
        try {
            ...
            for (BoneBlueprint boneBlueprint : bbModelConverter.getSkeletonBlueprint().getMainModel())
                if (!boneBlueprint.getBoneName().equals("hitbox") &&
                        ...
                        !boneBlueprint.getBoneName().equals("daedalus_autogenerated_root"))
                    assignBoneModelID(leatherHorseArmor, boneBlueprint);
        } catch (Exception e) {
           MessageUtil.log("Failed to parse model " + childFile.getName() + "! Warn the developer about this");
            e.printStackTrace();
        }
    }
